...
const downloadMediafire = async (url) => {
	if (!/mediafire.com/.test(url)) throw new Error('Invalid MediaFire URL')
	try {
		const res = await axios.get(url, { httpsAgent: unsafeAgent });
		const html = res.data.toString();
		let direct = null;

		// common patterns for direct link
		const hrefMatch = html.match(/href\s*=\s*"(https?:\/\/download[^"]+)"/i);
		if (hrefMatch) direct = hrefMatch[1];

		if (!direct) {
			const windowLoc = html.match(/window\.location(?:\.href)?\s*=\s*"(https?:\/\/[^"]+)"/i);
			if (windowLoc) direct = windowLoc[1];
		}

		if (!direct) {
			const kno = html.match(/kNO\s*=\s*"([^"]+)"/i) || html.match(/kNO\s*=\s*'([^']+)'/i);
			if (kno && kno[1]) {
				direct = kno[1];
			}
		}
		...
		if (!direct) {
			// try finding any download link pattern
			const any = html.match(/https?:\/\/download[^"'\s>]+/i);
			if (any) direct = any[0];
		}

		if (!direct) throw new Error('Direct download link not found');

		const filename = path.basename(new URL(direct).pathname).split('?')[0] || `mediafire-${Date.now()}`;
		const dest = path.join(ROOT_DIR, 'database', 'temp', `${Date.now()}-${filename}`);

		const streamRes = await axios.get(direct, { responseType: 'stream', httpsAgent: unsafeAgent });
		await new Promise((resolve, reject) => {
			const w = fs.createWriteStream(dest);
			streamRes.data.pipe(w);
			w.on('finish', resolve);
			w.on('error', reject);
		});

		const stat = fs.statSync(dest);
		return { path: dest, filename, size: bytesToSize(stat.size) };
	} catch (e) {
		throw e;
	}
}
